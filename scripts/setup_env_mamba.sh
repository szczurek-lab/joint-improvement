...
# Verify mamba is installed and available
if ! command -v mamba &> /dev/null; then
  echo "Error: mamba is not installed or not in PATH."
  echo "Please install mamba from: https://mamba.readthedocs.io/en/latest/installation.html"
  exit 1
fi
...
if [ -n "$ENV_DIR" ]; then
  ...
  if [ -d "$ENV_DIR" ]; then
    # mamba refuses to create into an existing non-conda directory (even if empty).
...
if [ "$ENV_EXISTS" = false ]; then
  if [ -n "$ENV_DIR" ]; then
    ...
    mamba create --yes --prefix "$ENV_DIR" -f "$ENV_YML"
  else
    # Name mode: create using the name from environment.yml
    mamba env create --yes -f "$ENV_YML"
  fi
fi

if [ "$WITH_DOCKING_GPU" = true ]; then
  echo "Installing Docking GPU additional dependencies (Boost)..."
  mamba install --yes "$ENV_SPEC" "$ENV_LOCATION" -c conda-forge "libboost=1.84.0" "boost-cpp=1.84.0"
fi

# Install PyTorch with CUDA 12.6 FIRST (before packages that depend on it)
mamba run "$ENV_SPEC" "$ENV_LOCATION" pip install torch torchvision --index-url https://download.pytorch.org/whl/cu126
...
mamba run "$ENV_SPEC" "$ENV_LOCATION" pip install -e ".[dev,docking]"
mamba run "$ENV_SPEC" "$ENV_LOCATION" pre-commit install

# Verify installation
if mamba run "$ENV_SPEC" "$ENV_LOCATION" python - <<'PY'; then
import sys
