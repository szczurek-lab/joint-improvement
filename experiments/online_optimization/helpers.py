...
def calculate_optimization_metrics(
    ...
    normalized_docking_scores = -objective_scores / 20
...
def save_solutions(
    ...
    current_solutions = [
        ...
        for smi, obj_vals in zip(smiles_list, objective_values_list)
    ]
...
def calculate_regression_metrics(
    ...
    predicted = inverse_transform(np.array(predicted, dtype=np.float32))
    true = inverse_transform(np.array(true, dtype=np.float32))
    mae = np.mean(np.abs(predicted - true))
    spearman_corr = spearmanr(predicted, true).correlation
