...
def gumbel_log_survival(x):
    r"""Computes log P(g > x) = log(1 - P(g < x)) = log(1 - exp(-exp(-x))) for a standard Gumbel.
...
class _TrieNode:
    ...
    def __init__(self, parent: Optional["_TrieNode"], index_in_parent: int | None) -> None:
        ...
        self.sbs_child_state_cache: list[tuple[sbs.State, bool]] | None = None
    ...
    def add_simple_advantage(self, mass: float) -> None:
        ...
        if not self.parent:  # is the root
            return
        self.parent.children_advantages[self.index_in_parent] = (
            self.parent.children_advantages[self.index_in_parent] + mass
        )
        self.parent.children_visit_counts[self.index_in_parent] = (
            self.parent.children_visit_counts[self.index_in_parent] + 1
        )
        self.parent.add_simple_advantage(mass)

    def obtain_locally_estimated_advantage(
        self,
        root_expectation: float,
        advantage_constant: float,
        beam_leaves: list,
        leaf_eval_fn: Callable,
    ):
        if (
            not self.parent or self.advantage_touched_flag
        ):  # is the root or it was already updated (and thus, also its ancestors)
            return
        ...
        if len(self.children):
            ...
            if len(shared_parent_leaves) >= 2:
                ...
                self.parent.children_advantages[self.index_in_parent] = self.parent.children_advantages[
                    self.index_in_parent
                ] + advantage_constant * (expectation_node - expectation_parent)
...
class IncrementalSBS:
    ...
    def __init__(
        ...
        leaf_evaluation_fn: Callable[[sbs.State], float] | None = None,
        ...
        self.root_nodes = [(_TrieNode(None, None), state) for state in initial_states]
        self.root_nodes_exhausted = [False] * len(
            self.root_nodes
        )  # will be True if all probability mass has been returned
    ...
    def perform_incremental_sbs(
        self,
        beam_width: int,
        num_rounds: int,
        log_prob_update_type: str = "reduce_mass",
        ...
        # Optional parallel meta: per-leaf (round_idx, leaf_rank_in_round)
        round_info_batch: list[list[tuple[int, int]]] = [[] for _ in range(len(self.root_nodes))]
        ...
        for round_idx in range(num_rounds):
            ...
            top_p_round = (
                1.0
                if num_rounds == 1
                else (1 - round_idx / (num_rounds - 1.0)) * min_nucleus_top_p + 1.0 * (round_idx / (num_rounds - 1.0))
            )
            round_beam_leaves_batch = sbs.stochastic_beam_search(
                child_log_probability_fn=self.wrap_child_log_probability_fn(
                    self.child_log_probability_fn, normalize_advantage_by_visit_count
                ),
                ...
                for i, beam_leaf in enumerate(beam_leaves):
                    ...
                    if not is_deterministic_round:
                        if log_prob_update_type == "gumbeldore":
                            ...
                            root_expectation = gumbel_without_replacement_expectation(
                                beam_leaves, self.leaf_evaluation_fn
                            )
                            leaf_node.obtain_locally_estimated_advantage(
                                root_expectation,
                                advantage_constant,
                                beam_leaves,
                                self.leaf_evaluation_fn,
                            )

                    client_beam_leaves.append(beam_leaf._replace(state=client_state))
                    # Store round info in the same order
                    round_info_batch[batch_idx].append((round_idx, i))
        ...
        if return_round_info:
            return leaves_batch, round_info_batch
        return leaves_batch
    ...
    @staticmethod
    def wrap_child_log_probability_fn(
        child_log_probability_fn, normalize_advantage_by_visit_count: bool
    ) -> Callable[[list[tuple[_TrieNode, sbs.State]]], list[np.ndarray]]:
        def wrapper_child_log_probability_fn(
            node_state_tuples: list[tuple[_TrieNode, sbs.State]],
        ) -> list[np.ndarray]:
    ...
    @staticmethod
    def wrap_child_transition_fn(
        child_transition_fn, memory_aggressive: bool
    ) -> Callable[
        [list[tuple[tuple[_TrieNode, sbs.State], int]]],
        list[tuple[tuple[_TrieNode, sbs.State], bool]],
    ]:
        def wrapper_child_transition_fn(
            node_state_action_index_pairs: list[tuple[tuple[_TrieNode, sbs.State], int]],
        ) -> list[tuple[tuple[_TrieNode, sbs.State], bool]]:
            ...
            for i, ((node, client_state), child_index) in enumerate(node_state_action_index_pairs):
                ...
                if node.children[child_index] is None or node.sbs_child_state_cache[child_index] is None:
                    ...
                    results[i] = (
                        (node.children[child_index], child_client_state),
                        child_is_leaf,
                    )
