...
Source: https://github.com/schwallergroup/saturn/blob/master/oracles/reward_shaping/reward_shaping_function.py
"""
import warnings
import numpy as np
import math

from joint_improvement.utils.chemistry.docking_utils.reward_shaping.function_parameters import RewardShapingFunctionParameters


class RewardShapingFunction:

    def __init__(self, parameters: RewardShapingFunctionParameters) -> None:
    ...
    def __call__(self, raw_property_values: np.ndarray[float]) -> np.ndarray[float]:
        ...
        try:
            ...
            raise ValueError(f"Oracle: {self.transformation_function} reward shaping function but not all parameters have been specified.")
    ...
    def step_transformation(
        ...
        def _step_formula(value: float, low: float, high: float) -> float:
            if low <= value <= high:
                return 1.0
            return 0.0
    ...
    def sigmoid_transformation(
        ...
        def _sigmoid(value: float, low: float, high: float, k: float) -> float:
            try:
    ...
    def reverse_sigmoid_transformation(
        ...
        def _reverse_sigmoid_formula(value: float, low: float, high: float, k: float) -> float:
            try:
    ...
    def double_sigmoid_transformation(
        ...
        def _double_sigmoid_formula(value: float, low: float, high: float, coef_div: float, coef_si: float, coef_se: float) -> float:
            try:
